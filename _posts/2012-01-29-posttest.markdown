...
There are two types of markup in : Output and Tag.
...
* `date` - reformat a date [syntax reference](http://.rubyforge.org/classes//StandardFilters.html#M000012)
...
`if / else` should be well known from any imaginable programming language.
 allows you to write simple expressions in the `if` or `unless` (and
